...
function showAverage() {
  ...
  //Value return
  if (media < 4) {
    resultAverage.innerText = `Your score average is: ${parseFloat(
      media
    ).toFixed(2)} \nAnd you're disapproved. It's a shame!`;

    resultAverage.setAttribute("class", "isDisapproved");
    ...
    resultAverage.setAttribute("class", "isAlert");
  } else if ((media >= 7) & (media <= 10)) {
    resultAverage.innerText = `Your score average is: ${parseFloat(
      media
    ).toFixed(2)} \nAnd you're approved. Congratulations!`;

    resultAverage.setAttribute("class", "isApproved");
  } else {
    resultAverage.innerText =
...
// event fired for each keyup typed, calling the function checkInputs and validating if there are still empty fields to active or not disabled button
...
// Variables getting events to keyup/blur and attribute the maxValue to id of inputs
...
// Function limiting minValue for number(average scores) on inputs after run the events above
function minNumber(min) {
  ...
  return function () {
    ...
    //For blocking multiple calls about blur and keyup
    running = true;
    //For fixing default value if value into input will be smaller that minValue(in this code the min is 0)
...
// to ensure when you click on button "Calculate" the page not be rendered again
const btnResultAverage = document.getElementById("btnCalculate");
if (btnResultAverage) {
  btnResultAverage.addEventListener("click", (event) => {
